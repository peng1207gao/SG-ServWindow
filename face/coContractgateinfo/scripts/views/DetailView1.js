...
coContractgateinfo.views.DetailView1 = function()
{
    ...
    function _initDataForm()
    {
        ...
        _form = new mx.datacontrols.DataForm({
	        ...
	        {	name: "busiunitid", caption: "联络线", editorType: "DropDownEditor",onchanged:_dropDownEditor_tieline_changed},
	...
	function _getContractId(){
		...
		if(me.getForm().editors.displaytype.getValue()!=3){
			me.getForm().entityContainer.setValue("displaytype",'3');
			me.getForm().editors.displaytype.setValue('3');
			_changed();
		}
		
		var items = restClient.getSync(coContractgateinfo.
    			mappath("~/rest/cocontractgateinfo/getTieline")).resultValue;	//获取购电方数据
...
//		me.getForm().editors.displaytype.removeItem(me.getForm().editors.displaytype.items[0]);
//		me.getForm().editors.displaytype.removeItem(me.getForm().editors.displaytype.items[1]);
	...
	function _dropDownEditor_changed() {
		...
		var items = restClient.getSync(coContractgateinfo.
    			mappath("~/rest/cocontractgateinfo/getTielinebyparticipantid"),
	...
	function _dropDownEditor_tieline_changed(){
		var tielineid = me.getForm().editors.busiunitid.getValue();
		var itemsGate = restClient.getSync(coContractgateinfo.
		mappath("~/rest/cocontractgateinfo/getGateidbytieline"),
		{tielineid:tielineid}).resultValue;	//获取购关口
