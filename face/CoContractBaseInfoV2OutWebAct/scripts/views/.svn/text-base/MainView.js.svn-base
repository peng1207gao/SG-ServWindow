$ns("CoContractBaseInfoV2OutWebAct.views");
...
CoContractBaseInfoV2OutWebAct.views.MainView = function()
{
...
//    	_initDataGrid2();//初始化第一个表格(统计信息)
     ...
     */
    function init(){
    	var items = restClient.getSync(CoContractBaseInfoV2OutWebAct.mappath("~/rest/coContractbaseinfoV2Act/init")).resultValue.items;
    	marketId = items[0];
    }
    ...
    function initLayout(){
    	//页面布局分为上下两部分
//    	me.mSplit = new mx.containers.HSplit({rows:"143,auto",borderThick:0});	//该控件将布局分为上下两部分
//    	me.addControl(me.mSplit);	//添加布局控件
    	
    	//布局上部分分为"业务查询"和"业务操作"两部分
    	
    	//业务查询区域
    	me.panel1 = new mx.containers.Panel({width:"100%",height:"80",title:"业务查询"});	//业务查询panel容器
...
//    	me.container1.addControl(me.component10); //添加下拉框
    	...
    	me.label12 = new mx.controls.Label({text:"结束时间：",width:"13%",textAlign:"right",right:4});	//结束时间标签
    	...
    	me.component4 = new mx.editors.DropDownEditor({displayMember:"name",valueMember:"value",width:"18%",
    		url:CoContractBaseInfoV2OutWebAct.mappath("~/../ETradePublicUtils/rest/baGencodeTypeList/getList?type=23")	//获取合同周期下拉框数据
    		}); //合同周期下拉框
    	me.container1.addControl(me.component4);	//添加下拉框
    	
    	//me.container1添加业务查询第二行的控件
//    	me.label5 = new mx.controls.Label({text:"合同状态",width:"80",textAlign:"right",right:4});	//合同状态标签
//    	me.container2.addControl(me.label5);	//添加标签
//    	me.component5 = new mx.editors.DropDownEditor({displayMember:"name",valueMember:"value",width:"190",
//    		url : CoContractBaseInfoV2OutWebAct.mappath("~/../ETradePublicUtils/rest/baGencodeTypeList/getList?type=27")	//获取合同状态下拉框数据
//    		}); //合同状态下拉框
//    	me.container2.addControl(me.component5);	//添加下拉框
//    	me.label7 = new mx.controls.Label({text:"购电方",width:"80",textAlign:"right",right:4});	//购电方标签
//    	me.container2.addControl(me.label7);	//添加标签
//    	me.component7 = new mx.editors.DropDownEditor({displayMember:"name",valueMember:"value",width:"190"});	//购电方下拉框
//    	me.container2.addControl(me.component7); //添加文本框
//    	setComponent7Items();	//给购电方下拉框赋值
//    	me.label8 = new mx.controls.Label({text:"售电方",width:"80",textAlign:"right",right:4});	//售电方标签
//    	me.container2.addControl(me.label8);	//添加标签
//    	me.component8 = new mx.editors.DropDownEditor({displayMember:"name",valueMember:"value",width:"190"}); //售电方下拉框
//    	me.container2.addControl(me.component8);	//添加下拉框
//    	setComponent8Items();	//给售电方下拉框赋值
//    	me.label9 = new mx.controls.Label({text:"业务场景",width:"80",textAlign:"right",right:4});	//业务场景标签
//    	me.container2.addControl(me.label9);	//添加标签
//    	me.component9 = new utils.dropDownEditor.MarketTree.MarketDropDownEditor(false,null,false); //业务场景下拉框
//    	me.component9.setWidth(190);
//    	me.container2.addControl(me.component9);	//添加下拉框
    	...
    	me.component13 = new mx.editors.DropDownEditor({displayMember:"name",valueMember:"value",width:"18%",autoMatchInput:true,
    		allowEditing:true}); //合同序列下拉框
    	//解决因为内容过长下拉框显示不全的问题
...
//    }
    ...
    function exportTable(){
    	...
    	util.setBaseUrl(CoContractBaseInfoV2OutWebAct.mappath("~/rest/coContractbaseinfoV2Act/excel")) ;
    	util.exportExcel();
    }
//    /**
//     * 初始化第一个表格
...
//            baseUrl : CoContractBaseInfoV2OutWebAct.mappath(restUrl),
...
//			// 构造查询属性。
//			alias: "CoContractBaseInfoV2OutWebActMainViewDataGrid2",
//			columns:[
...
//    }
    ...
    {
        var restUrl = "~/rest/coContractbaseinfoV2Act/findOutWebContractList";
        /* 初始化 EntityContainer */        
        var gridEntityContainer = new mx.datacontainers.GridEntityContainer({
            baseUrl : CoContractBaseInfoV2OutWebAct.mappath(restUrl),
            iscID : "-1", // iscID 是数据元素的统一权限功能编码。默认值为  "-1" ，表示不应用权限设置。
        ...
        _dataGrid = new mx.datacontrols.ComplexGrid({   
			// 构造查询属性。
			alias: "CoContractBaseInfoV2OutWebActMainViewDataGrid",
			columns:[
...
//			{	name: "col7", caption: "电价" , editorType: "TextEditor",align:"center",dataAlign:"center"	},
			...
			{	name: "contracttype", caption: "contracttype" , editorType: "TextEditor",align:"center",dataAlign:"center"	}
...
//			{	name: "col15", caption: "附件下载" , editorType: "TextEditor",align:"center",dataAlign:"center"	}
    ...
    function _init(){
    	...
    	var restClient = new mx.rpc.RESTClient();
    	var result = restClient.getSync(CoContractBaseInfoV2OutWebAct.mappath("~/rest/coContractbaseinfoV2Act/isfdqht"),{"params":JSON.stringify({"contracttype":contracttype})});
    	if(result==true){
    ...
    function setComponent7Items(){
    	...
    	var items = restClient.getSync(CoContractBaseInfoV2OutWebAct.
    ...
    function detailContact(p_item,$p_cell){
    	...
    	var htxlList = restClient.getSync(CoContractBaseInfoV2OutWebAct.mappath("~/rest/coContractbaseinfoV2Act/getHtxl"),{contractid:objId});
    	var htxlName = null;
