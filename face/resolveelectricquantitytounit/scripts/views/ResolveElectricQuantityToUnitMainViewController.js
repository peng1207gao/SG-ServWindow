...
//$import("resolveelectricquantitytounit.views.ResolveElectricQuantityToUnitDetailViewController");
//$import("resolveelectricquantitytounit.views.ResolveElectricQuantityToUnitMainView");

resolveelectricquantitytounit.views.ResolveElectricQuantityToUnitMainViewController = function()
{
    ...
    me.getView = function()
    {
        if (me.view == null)
        {
            me.view = new resolveelectricquantitytounit.views.ResolveElectricQuantityToUnitMainView({ controller: me });
        }
        return me.view;
    };
    ...
    me._getDetailFromView = function(){
    	if (_detailView == null)
        {
            var mvc = new resolveelectricquantitytounit.views.ResolveElectricQuantityToUnitDetailViewController();
...
//	    me.view.getDataGrid().load();
	...
	me.getEconUnitView = function(){
		...
		_econUnitView.contractids = dataGrid.filter;

		_econUnitView._initUnitItem(_econUnitView.contractids);
		var chooseUnitDataGrid = _econUnitView.getDataGrid();
		
		var data = {"contractids":_econUnitView.contractids.substring(15, _econUnitView.contractids.length - 1)};
		
		chooseUnitDataGrid.setFilter(data);
		...
		win.showDialog(function(chooseView){
			...
			if(view.returnstatus == "confirm"){//点击【确定】按钮，回调处理
				var _ResolveDataTable = me.getView().getResolveDataTable();
		    	...
		    	_ResolveDataTable.setFilter(JSON.stringify({"chooseunit":true, "checkeditems":itemsArray}));
