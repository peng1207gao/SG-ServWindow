...
conEnerMessByType.views.mainView=function(){
	...
	function _initHSplit1Area0(){
    	...
    	var queryPieBotton = new mx.controls.Button({ text: "查询",width:80,height:30,left:30,bottom:-5});//查询合同
	...
	function _initHSplit1Area1(){
		...
		var responseContent = restClient.getSync(conEnerMessByType.mappath("~/rest/conEnerMessByType/getChartPie"), { "params": JSON.stringify(jgParams)});
		me.conEnerMess = responseContent.resultValue.items[0];
		if(me.conEnerMess=="0"){
			alert("该业务场景不存在数据");
			me.conEnerMess="";
		}
		var chart = new FusionCharts(fusionChart.mappath("$/swf/Pie3D.swf"),"myChartId","100%","100%","0","1");//图表的显示
