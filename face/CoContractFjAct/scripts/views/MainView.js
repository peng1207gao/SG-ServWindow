...
CoContractFjAct.views.MainView = function()
{
     ...
     */
    function init(){
    	var items = restClient.getSync(CoContractFjAct.mappath("~/rest/coContractbaseinfoV2Act/init")).resultValue.items;
    	marketId = items[0];
    }
...
//    	me.addControl(me.mSplit);	//添加布局控件
    ...
    function initComponent(){
    	...
    	me.component4 = new mx.editors.DropDownEditor({displayMember:"name",valueMember:"value",width:"17%",
    		url:CoContractFjAct.mappath("~/../ETradePublicUtils/rest/baGencodeTypeList/getList?type=23")	//获取合同周期下拉框数据
    		}); //合同周期下拉框
...
//    	me.container2.addControl(me.component10); //添加下拉框
    	...
    	me.component5 = new mx.editors.DropDownEditor({displayMember:"name",valueMember:"value",width:"17%",
    		url : CoContractFjAct.mappath("~/../ETradePublicUtils/rest/baGencodeTypeList/getList?type=67")	//获取合同状态下拉框数据
    		}); //合同状态下拉框
    	...
    	me.button7 = new mx.controls.Button({text:"合同文件上传",left:60,onclick:fileUpload});	//合同导出按钮
...
//    	me.button10.on("click",me.controller._btnNew_onclick);
    ...
    function fileUpload(){
    	...
    	// 表单视图保存后事件处理。
		_detailView.getForm().entityContainer.off("saved", me.loadMasterDataGrid);
		_detailView.getForm().entityContainer.on("saved", me.loadMasterDataGrid);
    ...
    // 加载列表数据。
	me.loadMasterDataGrid = function(e)
	{
		_dataGrid.load();
	}
...
//			// 构造查询属性。
//			alias: "CoContractFjActMainViewDataGrid2",
//			columns:[
...
//    }
    ...
    function _initDataGrid()
    {
        ...
        _dataGrid = new mx.datacontrols.ComplexGrid({   
			// 构造查询属性。
			alias: "CoContractFjActMainViewDataGrid",
			columns:[
...
//			{	name: "col14", caption: "维护单位" , editorType: "TextEditor",align:"center",dataAlign:"center"	},
    ...
    function _init(){
    	...
    	//合同类型为发电权合同时 改表头名称
    	var restClient = new mx.rpc.RESTClient();
    	var result = restClient.getSync(CoContractFjAct.mappath("~/rest/coContractbaseinfoV2Act/isfdqht"),{"params":JSON.stringify({"contracttype":contracttype})});
    	if(result==true){
    ...
    function setComponent7Items(){
    	...
    	var items = restClient.getSync(CoContractFjAct.
    ...
    function detailContact(p_item,$p_cell){
    	...
    	var htxlList = restClient.getSync(CoContractFjAct.mappath("~/rest/coContractbaseinfoV2Act/getHtxl"),{contractid:objId});
    	var htxlName = null;
