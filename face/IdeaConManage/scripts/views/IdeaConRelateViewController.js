...
IdeaConManage.views.IdeaConRelateViewController = function()
{
    ...
    me.getView = function()
    {
        if (me.view == null)
        {
            me.view = new IdeaConManage.views.IdeaConRelateView({ controller: me });
        }
        return me.view;
    };
...
//	    me.view.getDataGrid().load();
    ...
    me._showContractDetailView = function(p_item){
    	...
    	_detailView.sceneType = "ideaContract";//获取树结构的参数，标识获取意向性合同树结构
    	_detailView._contractTree.load({"contractid": _detailView.objID, "sceneType":"ideaContract"});//初始化树结构
	    //显示详细信息页面
