package com.sgcc.dlsc.contractManage.settleaccountsassist;
...
import com.sgcc.dlsc.contractManage.settleaccountsassist.bizc.ISettleaccountsassistBizc;
...
@Controller
@RequestMapping("/settleAccountsAssist") //根据po类名生成
public class SettleaccountsassistController {
	
    @Autowired
    private ISettleaccountsassistBizc settleaccountsassistBizc;
	/**
	 * 保存
	...
	List<CoContractbaseinfoVO> save(@ItemsRequestBody List<Map> list){ 
		try{
    		List<CoContractbaseinfoVO> voList = settleaccountsassistBizc.saveOrUpdate(list);
    		return voList;
		}catch(Exception e){
	...
	Object delete(@IdRequestBody IDRequestObject idObject) {
		try {
			settleaccountsassistBizc.remove(idObject);
		} catch (Exception e) {
	...
	QueryResultObject query(@QueryRequestParam("params") RequestCondition params) {
		QueryResultObject result;//定义返回值
 		try {
			result = settleaccountsassistBizc.findCoContractList(params);
		} catch (Exception e) {
			e.printStackTrace();
			throw new RestClientException("程序异常！");
		}
	...
	QueryResultObject query(@PathVariable String id) {
		try {
			return settleaccountsassistBizc.queryById(id);
		} catch (Exception e) {
			throw new RestClientException("程序异常！");
		}
	}
	...
	/**
	 * 注入逻辑构件
	 * @param settleaccountsassistBizc
	 */
	public void setSettleaccountsassistBizc(ISettleaccountsassistBizc settleaccountsassistBizc) {
		this.settleaccountsassistBizc = settleaccountsassistBizc;
	}
}
